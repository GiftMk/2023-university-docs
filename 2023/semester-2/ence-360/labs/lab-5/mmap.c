...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < repeats; ++i) {
    ...
    if (dst_map == MAP_FAILED) handle_error("mmap dst");

    // Copy data from source to destination using memcpy
    memcpy(dst_map, src_map, size);

    // Unmap memory mappings to ensure data is written
    if (munmap(src_map, size)) handle_error("munmap src");
    if (munmap(dst_map, size)) handle_error("munmap dst");
